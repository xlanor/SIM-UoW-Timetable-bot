...
def test_timetable_parser():
    ...
    for c in list_of_class:
        ...
        assert (
            c.class_type is not None
        ), f"A retrived class, {c.name} has a null class type"
        assert (
            c.start_time is not None
        ), f"A retrieved class, {c.name} has a null start_time"
        assert (
            c.end_time is not None
        ), f"A retrieved class, {c.name} has a null end time"
        # All time should be formatted as
        # 03:00PM | 12:30AM
        assert (
            len(c.start_time) == 7
        ), f"Class {c.name} of type {c.type} has a start time of an invalid length!"
        assert (
            len(c.end_time) == 7
        ), f"Class {c.name} of type {c.type} has an end time of an invalid length!"
