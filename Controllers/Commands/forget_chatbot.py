    ...
    try:
        ...
        if db_interface.user_exist(uid):
            ...
            if delete_result.deleted_count == 1:
                message_array.append(f"Details for {uid} has been removed from the database\n")
                message_array.append("You can now register again using /register")
            else:
                message_array.append("An Unknown error has occured! Kindly inform @fatalityx")
            message = "".join(message_array)
            update.message.reply_text(message,parse_mode='Markdown')
        else:
            message_array = [f"Unable to find your telegram ID {uid} in the database!\n"]
